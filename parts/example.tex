...
which can also be seen as the IG in \pref{fig:runningBRN-1}, as its fourth component is an empty set.
...
As there is no cycle in the bounces of $b$, and given \pref{thm:param_K}, it hence comes:
...
The current implementation can successfully solve the examples given in \pref{fig:runningPH-1} (with an incomplete Parametrization) and \pref{fig:runningPH-2}. It can also handle bigger models such as ERBB receptor-regulated G1/S transition \todo{ref? \cite{PMR10-TCSB}?} which contains 20 components, and T-cells receptor \todo{name? ref?} which contains 40 components.
the following table gives the execution time of the \texttt{ph2thomas} tool for these models, on a personal laptop \todo{perfs}.

\begin{center}
\begin{tabular}{c|c|c|}
  & Without enumeration & With enumeration
\\ \hline
ERBB transition & 910 ms & 990 ms
\\ \hline
T-cells receptor & 640 ms & 680 ms
\\ \hline
\end{tabular}
\end{center}

As the inferred Parametrization is complete in both cases, the admissible Parametrizations enumeration does not significantly change the execution times.

\todo{Pourquoi t(TCRSIG) < t(ERBB)??? Pourtant on a le même nombre de sortes coopératives et d'arcs inférés, mais plus de composants dans TCRSIG...}
