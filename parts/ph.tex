...
We recall here the definition and semantics of the Process Hitting, and its usage to model
cooperation between concurrent components.
Two examples of Process Hitting modeling a BRN at different level of abstraction are given.
They serve as running examples in the rest of this article.

\medskip

A Process Hitting (\pref{def:PH}) gathers a finite number of concurrent \emph{processes}
...
\begin{example*}
\pref{fig:runningPH-1} represents a Process Hitting $(\PHs,\PHl,\PHa)$ with
$\PHs = \{a,b,c\}$,
$\PHl_a = \{a_0,a_1,a_2\}$,
$\PHl_b = \{b_0, b_1\}$,
$\PHl_c = \{c_0, c_1\}$, and
...
This example Process Hitting actually models a BRN where the component $a$ has three qualitative
...
we then model an over-approximation of the possible actions.
\end{example*}
...
expressed in Process Hitting by building a \emph{cooperative sort}.
...
Each process of sort $b$ and $c$ hit $bc$ to make it bounce to the process reflecting their current
state (e.g., $\PHfrappe{b_1}{bc_{00}}{bc_{10}}$ and $\PHfrappe{b_1}{bc_{01}}{bc_{11}}$).
Then, it is the process $bc_{11}$ which hits $a_1$ to make it bounce to $a_2$ instead of the
independent hit from $b_1$ or $c_1$.
...
Process Hitting resulting from the refining of the one in \pref{fig:runningPH-1} by the
...
When the number of cooperating process is large, it is possible to chain several cooperative sorts
...
reflecting the presence of $bc_{11}$ and $d_1$.  Such constructions are helpful in Process Hitting
as the static analysis of dynamics developed in \cite{PMR12-MSCS} does not suffer on the number of
sorts, but on the number of processes within a single sort.

While the construction of cooperation in Process Hitting allows to encode any boolean functions
...
The resulting behaviour is then an over-approximation of the realisation of an instantaneous
cooperation.

\begin{example*}
The Process Hitting in \pref{fig:runningPH-2} results from the refining of the Process Hitting in 
\pref{fig:runningPH-1} where several cooperations have been specified.
...
Hence, this Process Hitting expresses a BRN where $a$ requires both $b$ and $c$ active to reach its
highest level, and $a$ do not become inactive unless both $b$ and $c$ are inactive.
\end{example*}
