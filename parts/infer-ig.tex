...
Assuming $\PHpredecgene{a}\neq\emptyset$, the inference of the self-influence of $a$ requires a more
careful treatment (\pref{eq:edges-inference-b}).
We first exclude the case where $a$ is its only regulator and its focal processes are a non-empty
interval: $a$ evolves to a constant interval.
Then, we look for a configuration of all its regulators where the level of $a$ has an impact
on the evolution of the sort $a$.
Given $a_i,a_{i+1}\in L_a$, we pick $a_j\in\focals(a,\{a_i\},\ctx'(\sigma\{a_i\}))$ and
$a_k\in\focals(a,\{a_{i+1}\},\ctx'(\sigma\{a_{i+1}\}))$, 
where $\ctx'(\sigma)$ is the context of all sorts hitting the sort $a$ for the configuration
$\sigma$ (\pref{eq:ctx-sigma2}).
...
Finally, if there is configuration such that the focal level of any $a_i\in L_a$ is $a_i$ itself, we
conclude that $a$ is auto-activating as increasing its level increases the resulting level.
\begin{equation}
\ctx'(\sigma) \DEF \{ \sigma[a] \} \cup \{ c[\sigma] \mid c \in \PHdirectpredec{a}\cap\Gamma \}
\cup \{ \upsilon(\sigma) \mid \upsilon\in\PHdirectpredec{a} \wedge  \upsilon \notin\Gamma \}
\label{eq:ctx-sigma2}
\end{equation}
