...
We aim at inferring that $b$ actives (inhibits) $a$ if there exists a configuration where increasing
...
$b\neq a$.
Given a configuration $\sigma\in\configs b$, we define in \pref{eq:bounces}
$\f{bounces}(a_i,\ctx(\sigma))$ as the set of bounces that a given process $a_i$ can make in the
context $\ctx(\sigma)$.
We note $\sigma\{b_i\}$ the configuration $\sigma$ where the process of sort $b$ has been replaced
by $b_i$.
If there exists $b_i,b_j\in L_b$ with $i < j$, such that one bounce in 
$\f{bounces}(\sigma[a], \ctx(\sigma\{b_i\}))$
has a lower (resp. higher) level that one bounce in
$\f{bounces}(\sigma[a], \ctx(\sigma\{b_j\}))$, then
$b$ as positive (resp. negative) influence on $a$ with a maximum threshold $k=j$.

\begin{equation}
\f{bounces}(a_i,\ctx(\sigma)) \DEF 
\begin{cases}
\{ a_i \} \quad\text{if }\nexists \PHfrappe{c_k}{a_i}{a_j}\in \PHa \wedge c_k \in \ctx(\sigma) \\
\{ a_j \mid \exists \PHfrappe{c_k}{a_i}{a_j}\in \PHa \wedge c_k \in \ctx(\sigma) \}
& \text{otherwise.}
\end{cases}
\label{eq:bounces}
\end{equation}

The inference of the self-influence of $a$ requires a global overview of the evolution of each level
...
This latter case implies that $a$ has both a positive and negative influence on it.
\begin{equation}
\epsilon(a_i, \sigma) \DEF
\begin{cases}
\varnothing & \text{if }\f{bounces}(a_i,\ctx(\sigma\{a_i\}))=\{ a_i \} \\
+ &  \text{if }\f{bounces}(a_i,\ctx(\sigma\{a_i\})) = \{ a_{i+1} \}\\
- &  \text{if }\f{bounces}(a_i,\ctx(\sigma\{a_i\})) = \{ a_{i-1} \}\\
\pm & \text{otherwise.}
\end{cases}
\label{eq:epsilon}
...
We define the set of positive (resp. negative) influences $\hat{E}_+$ (resp. $\hat{E}_-$) by:
\begin{align}
\begin{split}
\forall b\in\Gamma, b\neq a, \\
b\xrightarrow k a \in \hat{E}_s & \Longleftrightarrow
        \exists b_i,b_j\in \PHl_b, i < j, \exists \sigma\in\configs b \\
& \qquad\qquad
        \exists a_{k}\in\f{bounces}(\sigma[a],\varsigma(\sigma\{b_i\})), \\
& \qquad\qquad
        \exists a_{l}\in\f{bounces}(\sigma[a],\varsigma(\sigma\{b_j\})), \\
& \qquad\qquad\qquad
                        s = \f{sign}(l - k) \wedge k = j
\end{split}
...
& \qquad \wedge (\exists\sigma\in\configs a,
                (\exists a_i\in L_a, \epsilon(a_i,\ctx(\sigma)) = \pm)\\
& \qquad\qquad \vee
        (\exists a_i,a_j\in L_a, i < j, \\
& \qquad\qquad\qquad
                \epsilon(a_i,\ctx(\sigma)) = \bar s \wedge \epsilon(a_j,\ctx(\sigma)) = s \wedge k = j) \\
& \qquad\qquad \vee
        ((\epsilon(a_0, \ctx(\sigma)) = \bar s \vee \epsilon(a_{l_a}, \ctx(\sigma)) = s) \wedge k = l_a) \\
& \qquad\qquad \vee
        (\epsilon(a_0, \ctx(\sigma)) = \epsilon(a_{l_a}, \ctx(\sigma)) = \varnothing \\
& \qquad\qquad\qquad       \wedge \forall a_i\in L_a, \epsilon(a_i, \ctx(\sigma))\in \{\varnothing, s\} \\
& \qquad\qquad\qquad       \wedge \exists a_j\in L_a, \epsilon(a_j, \ctx(\sigma)) = s \wedge k = j)
                )
\end{split}
\end{align}
...
E_+ &= \{a \xrightarrow{t} b \mid \nexists a \xrightarrow{t'} b \in \hat{E}_-
  \wedge t = \min_{a \xrightarrow{k} b \in \hat{E}_+} (k)\} \\
E_- &= \{a \xrightarrow{t} b \mid \nexists a \xrightarrow{t'} b \in \hat{E}_+
  \wedge t = \min_{a \xrightarrow{k} b \in \hat{E}_-} (k)\} \\
...
Finally, we note that some aspects of the inference of self-influences are arbitrary and may have
impact on the compatible parametrizations presented in the next section.
