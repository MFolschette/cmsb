...
& \qquad \wedge (\exists\sigma\in\configs a,
                (\exists a_i\in L_a, \epsilon(a_i,\sigma) = \pm)\\
& \qquad\qquad \vee
        (\exists a_i,a_j\in L_a, i < j, \\
& \qquad\qquad\qquad
                \epsilon(a_i,\sigma) = \bar s \wedge \epsilon(a_j,\sigma) = s \wedge k = j) \\
& \qquad\qquad \vee
        ((\epsilon(a_0, \sigma) = \bar s \vee \epsilon(a_{l_a}, \sigma) = s) \wedge k = l_a) \\
& \qquad\qquad \vee
        (\epsilon(a_0, \sigma) = \epsilon(a_{l_a}, \sigma) = \varnothing \\
& \qquad\qquad\qquad       \wedge \forall a_i\in L_a, \epsilon(a_i, \sigma)\in \{\varnothing, s\} \\
& \qquad\qquad\qquad       \wedge \exists a_j\in L_a, \epsilon(a_j, \sigma) = s \wedge k = j)
                )
\end{split}
\end{align}
