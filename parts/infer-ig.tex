...
\todo{in other words, if emptyset -> there exists a set of actions in $\PHa(S_a,\ctx)$ that can be
played as many number of times as we want (cycle);
	if non-emptyset -> all succession of actions in $\PHa(S_a,\ctx)$ lead $a$, in a bound number of
	steps, either to a process in $S_a$ that is not hit by processes in $\ctx$, or to a process in
	$L_a\setminus S_a$}
...
This is analogous to standard IG inferences from boolean and discrete maps \todo{refs adrien}.
...
+ &  \text{if }\forall a_l \in \f{bounces}(a_i,\ctx(\sigma)), l > i\\
- &  \text{if }\forall a_l \in \f{bounces}(a_i,\ctx(\sigma)), l < i\\
\pm & \text{otherwise.}
\end{cases}
\label{eq:epsilon}
