...
We first infer all existing influences between genes, given a certain threshold. These influences are split into positive and negative ones, and represent possible edges in the final IG.
...
& \qquad\qquad\qquad
                \epsilon(a_i,\ctx(\sigma)) = \bar s \wedge \epsilon(a_j,\ctx(\sigma)) = s \wedge t = j) \\
& \qquad\qquad \vee
        ((\epsilon(a_0, \ctx(\sigma)) = \bar s \vee \epsilon(a_{l_a}, \ctx(\sigma)) = s) \wedge t = l_a) \\
& \qquad\qquad \vee
...
& \qquad\qquad\qquad       \wedge \exists a_i\in L_a, \epsilon(a_i, \ctx(\sigma)) = s \wedge t = 1)
                )
\end{split}
\end{align}
