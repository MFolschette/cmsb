...
This subsection assumes a global Process Hitting $(\PHs,\PHl,\PHa)$ and the related inferred IG $(\Gamma, E_+, E_-)$, on which the parametrization inference is to be performed.
...
\begin{example*}
\todo{Illustrate the definitions}
\end{example*}

Given the \pref{def:param_K}, we see that in some cases, the inference of the desired parameter is impossible.
...
\end{align*}
where:
\[[i_1 ; i_2] <_{[]} [j_1 ; j_2] \Leftrightarrow (i_1 \leq j_1 \wedge i_2 < j_2) \vee (i_1 < j_1 \wedge i_2 \leq j_2)\]
\end{property}
...
\end{align*}
where:
\[[i_1 ; i_2] \leq_{[]} [j_1 ; j_2] \Leftrightarrow (i_1 \leq j_1 \wedge i_2 \leq j_2)\]
\end{property}
